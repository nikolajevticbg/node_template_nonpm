        ...
        _data.read('users', phone, function(err, data) {
            if (!err && data) {
                _data.delete('users',phone, (err) => {
                    if (!err) {
                        // Now we need to delete each of the checks asociated with the user
                                    ...
                                    if (err) {
                                        ...
                                        if (checksDeleted == checksToDelete ) {
                                            if (!deletionErrors) {
                                                ...
                                                callback(500, {'Error' : 'Errors encoutered attempting to delete user checks'});
                                            }
                                        }
                                    }
                                })
...
handlers._checks.put = function(data, callback) {
    ...
    if (id) {
        // Check to make sure that we have one optional field
        if (protocol || url || method || successCodes || timeoutSeconds) {
            // Lookup to check
            _data.read('checks', id, (err, checkData) => {
                if (!err && checkData) {
                    ...
                    callback(500, {'Error' : 'Check id did not exist'})
                }
            });
                ...
                handlers._tokens.verifyToken(token, checkData.userPhone, function(tokenIsValid) {
                    if (tokenIsValid) {
                        ...
                        _data.delete('checks', id, (err) => {
                            if (!err) {
                                // Lookup the user to delete the check
                                _data.read('users', checkData.userPhone, function(err, userData) {
                                    if (!err && userData) {
                                        ...
                                        if (checkPosition > -1) {
                                            ...
                                            // When check removed save user data again
                                            _data.update('users', checkData.userPhone, userChecks, (err) => {
                                                if (!err) {
                                                    callback(200);
                                                } else {
                                            ...
                                            callback (500, {'Error' : 'Could not find the check on users object and could not remove it'});
                                        }
                                    } else {
